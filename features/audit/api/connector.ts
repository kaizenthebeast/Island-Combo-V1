// Typed connector for the audit API — mirrors GET /api/audit 1:1.
...
export type AuditQueryParams = {
  type?: AuditCategory   // optional entity filter; omitted = all types
  page?: number
  limit?: number
  from?: string
  to?: string
...
export const getAuditLogs = (params: AuditQueryParams = {}): Promise<AuditPage> => {
  ...
  return apiFetch<AuditPage>(`/api/audit${qs ? `?${qs}` : ''}`)
}
